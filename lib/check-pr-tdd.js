...
function checkPrTDD(context) {
  ...
  .then(() => getSpecsAndTests())
  .then(specsAndTests => checkSpecsAgainsTests(specsAndTests))

  function checkSpecsAgainsTests(spectsAndTests) {
    const specs = spectsAndTests[0];
    const tests = spectsAndTests[1];

    return Promise.resolve(() => {
      if (_.isEqual(specs.sort(), tests.sort())) {
        // good
      } else if (specs.length > tests) {
        // to many specs
      } else {
        // to many tests
      }
  ...
  function getSpecsAndTests() {
    return Promise.all([
      getSpecsFromIssue(context),
